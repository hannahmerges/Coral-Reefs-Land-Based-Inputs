...
```{r}
### Was inspired by DMB kringing plots but maptools packages is no longer available in CRAN ###
### Updated by HEM 08/23/2025 using ggmap and ggspatial packages with API from google ####
...
nutrients <- read_csv(here("Data", "AugNutrient_Processed_CV.csv")) ## from DMB. -- need to have lat and long info so used previous dataset with measurements from all the sites, since I did not collect water quality data at each point during my study 
...
nutrients_V <- nutrients %>% 
  ...
  dplyr::mutate(Silicate_umolL = round(Silicate_umolL, digits = 0)) ## round the silicate values so they are integers

nutrients_C <- nutrients %>% 
  filter(Location=="Cabral") %>% 
  dplyr::mutate(Silicate_umolL = round(Silicate_umolL, digits = 0))
...
# Plot it
Varari_map1 <- ggmap(Varari_map) 

Varari_map1
ggsave(here::here("Outputs", "Publication", "V_map_site.jpg"), 
       width=12, height=8)
...
Varari_sites <- ggmap(Varari_map)+
  geom_point(data = nutrients_V,
             aes(x = lon, y = lat, color = round(Silicate_umolL, digits = 0)),
             size = 6) + 
...
Varari_tile2 <- image_scale(varari_tile, "800x600") ##crop the image so it is not as large
...
# Plot it
Cabral_map1 <- ggmap(Cabral_map) 

Cabral_map1
ggsave(here::here("Outputs", "Publication", "C_map_site.jpg"), 
       width=12, height=8)
...
Cabral_sites <- ggmap(Cabral_map)+
  geom_point(data = nutrients_C,
             aes(x = lon, y = lat, color = round(Silicate_umolL, 0)),
             size = 6) + 
  scale_color_gradientn(colors = c("seashell", "lightpink", "indianred", "coral4"), 
                        trans="log") +
  annotation_scale(bar_cols = c("ivory2", "ivory3"),
...
Cabral_tile2 <- image_scale(cabral_tile, "800x600") ## crop the image so it is not as large
